...
import { useAddComment, useComments } from "src/state/Article";
...
export function ArticleViewerModal({
  ...
  const articleComments = React.useMemo(
    () => comments.data?.filter((comment) => comment.article.id === article.id),
    ...
    <>
      <Dialog fullWidth maxWidth="md" open onClose={onClose}>
        ...
        <DialogContent>
          ...
          <x.small px="1rem">
            Last Updated: {format(article.updatedAt, dateFormat)}
          </x.small>{" "}
          <Box p="1rem">
